...
// Propósito  : LLamadas Epson DLL, Ejecutar comandos
// Creado Por :
// Llamado por:
// Aplicación :
// Tabla      :
...
//  cFileLog:="TEMP\"+DTOS(oDp:dFecha)+"_"+LSTR(SECONDS())+"_"+cCmd+".LOG"
  ...
  // Otro Comando, enviado con parámetros
  IF !oEpson:lRuned
...
FUNCTION PFRUNCMD(cFunc,cPar1,cPar2,cPar3,cPar4,cPar5,cPar6,cPar7,cPar8,cPar9)   
  ...
  IF ValType(uResult)="C" .AND. uResult="ER"
    oEpson:lError:=.T.
    // oEpson:oFile:AppStr("Función:"+cFunc+", Parámetro:"+cPar1+", Respuesta="+uResult+CRLF)
  ENDIF
